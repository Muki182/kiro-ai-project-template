...
│   ├── templates/              # 共享模板（生成 spec/hook 的基础）
│   │   ├── spec-template.md    # Spec 文件统一模板
│   │   └── hook-template.yml   # Hook 文件统一模板
│   └── shared/                 # 共享配置
│       └── python-excludes.yml # Python 项目通用排除模式
├── scripts/
│   ├── create-spec.sh      # 从模板生成新 spec
│   └── create-hook.sh      # 从模板生成新 hook
├── docs/
...
```bash
./scripts/create-spec.sh 03 "你的模块名称" "01-architecture.md" "@你的用户名"
```

或参考 `.kiro/templates/spec-template.md` 手动创建。核心要素：
- **需求列表**：编号化、可验证的功能需求
