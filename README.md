...
│   │   └── 03-security.md     # 安全需求 spec
│   └── hooks/
│       ├── doc-sync.yml        # 代码变更后自动同步文档
│       ├── spec-validate.yml   # 实现变更后校验与 spec 的一致性
│       ├── test-trigger.yml    # 关键文件变更后触发测试建议
│       └── security-check.yml  # 代码变更后安全审查
├── .pre-commit-config.yaml     # Git 提交前安全检查（detect-secrets, bandit 等）
├── docs/
...
### `security-check.yml`
**触发条件**：`src/` 下 Python 文件或 `config/` 下 YAML 文件变更
**行为**：检查常见安全漏洞（硬编码密钥、注入漏洞、不安全反序列化、CORS 配置、认证缺失等），输出安全审查报告
